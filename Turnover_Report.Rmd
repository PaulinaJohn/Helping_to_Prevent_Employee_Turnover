...
I looked at the proportion of employees who left compared to those who haven't.
...
I proceeded to look at the relationships between variables in the dataset. The relationship between Salary and other variables is an interesting one I wanted to see but, it was in character type. We can only compare relationships between numeric variables.
...
Then I checked the relationships between columns using a heatmap. Note that to create a heatmap in ggplot2, you need long data, as opposed to wide data format. This is where the melt function from the `reshape` package comes in.
...
```{r collecting data for only employees who have left into another dataframe}
